...
type Source = {
  read: typeof readPrLinkOverride
  detect: typeof detectPrLink
}

const source: Source = {
  read: readPrLinkOverride,
  detect: detectPrLink,
}

export function prStatusHandler(input: Source = source) {
  return Effect.fn("Cli.pr.status")(function* () {
    const ctx = yield* InstanceRef
    if (!ctx) return yield* fail("Could not load instance context")

    const override = yield* Effect.promise(() => input.read(ctx.worktree))
