...
- **`kilo-upstream`**: Fork of Kilo-Org/kilocode-legacy `main` branch - tracks upstream repository
- **`main-vertex`**: **Current project MAIN branch** - contains project-specific development

**All development work should be based on `main-vertex`, not `kilo-upstream`.**
...
1. Update branch `kilo-upstream` from upstream latest (`Kilo-Org/kilocode-legacy` `main`).
2. Merge latest `kilo-upstream` into `main-vertex`.
